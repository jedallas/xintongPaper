...
Researchers have proposed some technologies to underlie an indoor positioning system (IPS), including ultrasound\cite{b4}, RFID \cite{b5}. Visible lights communication\cite{b6} is another advanced trial in academia. While wireless signal-based indoor positioning systems, such as Bluetooth\cite{b7}\cite{b8} and WiFi\cite{b9}\cite{b10}, have made a great success in industry.
...
Basically three major kinds of principles are convincing: fingerprinting by Received Signal Strength Indication (RSSI) \cite{b11}\cite{b12},
...
K-Nearest Neighbor algorithm has been revealed as the most suitable model during positioning\cite{b13}.
An improved precision yielded by the model in conjunction with support vector machine, ensemble SVR, and artificial neural network has been demonstrated, as compared to an independent model\cite{b14}.
The results show that the performance of K-Nearest Neighbors, decision tree and Support Vector Machine (SVM) outperform other indoor localization methods both in computation time and precision\cite{b15}.
Also, the combination of K*algorithm\cite{b16} and Radial Basis Function (RBF) Regression are demonstrated as the most accurate model among 20 machine learning algorithms\cite{b17}.
...
necessary, such as predefining testing points and deploying the iBeacon base
station.??????????????????? (what is a base station)
...
Later, another experiment took place in a real supermarket with 36 meter wide and 50 meter long. Totally,
92 iBeacon devices were deployed and 31 testing points was selected. %, shown in Fig.\ref{fig:4}.
...
The Android App, \textit{iBeacon signal collector}, ran on a Huawei Honor 9 smartphone. In the offline signal collection phase, the collector stayed on each testing points for 2 minutes to capture iBeacon information until traversing all testing points. The iBeacon information was logged into a csv file stored on the server. In each recording instance, six attributes were logged, including time, iBeacon device name, BSSID (MAC address), Minor value, RSSI and testing point id.

In the preprocessing stage, the system applied EWMA technique with various $\alpha$ values on the raw dataset, and converted it into the acceptable input format for the machine learning classifier.
After preprocessing, an ensemble learning algorithm was adopted to train a localization estimation model on the entire dataset.
...
We examined the performances of 9 machine learning algorithms and the ensemble learning algorithm on our dataset using cross-validation.
Table \ref{tab:1} manifests the accuracy for each algorithm and that of the ensemble algorithm.
Here the ensemble learning algorithm achieved a high accuracy of 94.10\%.
...
\label{fig:5}
\end{figure}
% 横坐标是Error distance. 纵坐标是Accuracy。你要把工作做细，别捉急。概念要清晰。
...
Surprisingly, the 95\% errors of the three best systems are within 0m, 0.5m, and 1.5m respectively.
...
Average-$\alpha$ was set as 600 ms and 900 ms for two round of analyses. Table\ref{tab:4} shows the results.

Unlike the previous experiments, the average datasets outperform corresponding ensemble datasets.
Although amazing high accuracy has been achieved by those average datasets,
ensemble datasets still perform an excellent matching between the newly observed
fingerprints and existing fingerprints. ??????????
%and show good robustness which average datasets sometimes can’t show.  ??????
%meaning?
...
somewhere, an iBeacon device suddenly turns defunct for some reason so that it doesn't broadcast any signal.
...
That is to say, when iBeacon devices cannot receive signal strengths (Broken) or
display significant differences between observed values and existing values in
the radio map which is established as the predicted positioning points (Taken),
the system must detect it utilizing some mechanisms and drop these iBeacon
devices. ?????????????????????????? not clear what u r saying. too long sentence.

\begin{table}[htbp]
\caption{ACCURACY OF POSITIONING SYSTEM IN EXPERIMENT 2 WITH AN IBEACON LOST,
    what is 900ms explaining???????????????????}
\begin{center}
\begin{tabular}{|l|c|c|}
\hline
...
The system usually has an excellent performance when evaluating on the entire training data, however it can not perform extraordinary when evaluating on the testing data.
...
That is to say, the instance with a notation "3" is hard to make an accurate prediction using data augmentation. 

One step further, we tried to improve by a weighted fusion algorithm: 
we consider the final prediction by using its respective weights multiplies its
corresponding results of the same record in different datasets. (??? meaning?)
...
detection. (so what shall we do next?)
