...
necessary, such as predefining testing points and deploying the iBeacon devices.??????????????????? (what is a base station)%base station 基站 -》 devices
...
In order to compute the trend on the drastically fluctuating raw data, EWMA is adopted to obtain a much smoother curve, which can be used for amending the fluctuating and finding a hidden pattern.
...
Our first study case took place in a spacious lab with 14 meter wide and 18 meter long. %an->a
Totally, 12 testing points were predefined in the lab.
Two rounds of experiments were conducted in the lab, which had 23 iBeacon devices and 9 iBeacon devices, respectively. The layout of lab and distribution of multiple iBeacon devices are shown in Fig.\ref{fig:2} and Fig.\ref{fig:3}.
...
We examined the performances of 9 machine learning algorithms perspectively and the ensemble learning algorithm on our dataset using 10-fold cross-validation in the metrics of accuracy.
Table \ref{tab:1} manifests the accuracy for each algorithm and that of the ensemble algorithm.
Here the ensemble learning algorithm achieved a significantly high accuracy of 94.10\%.
...
\label{fig:5}
\end{figure}
% 横坐标是Error distance. 纵坐标是Accuracy。你要把工作做细，别捉急。概念要清晰。 (已改）
...
Surprisingly, the 95\% errors of the three best systems are within 0m, 0m, and 0.5m respectively.
...
%应该是interval was set as 600 ms and 900 ms     Average-$\alpha$  里面$\alpha$  是0.7 0.9 0.95 ....
...
% observed signal strengths and existing signal strengths. Besides, ensemble datasets can alleviate the overfittings significantly which is discussed later in the online phase subsection.
...
somewhere, or an iBeacon device suddenly turns defunct for some reason so that it doesn't broadcast any signal.
...
the radio map which has been established before (Taken),
the system must detect it utilizing some mechanisms such as classifies based on machine learning and drop these iBeacon
devices. ?????????????????????????? not clear what u r saying. too long sentence.

%ibeacon 坏了就收不到(broken)  被小孩子拿到别的地方收到的信息就和之前数据库里面的信息不匹配 （taken)
%所以我们需要某种机制可以distinguish这个ibeacon是不是不正常 然后去除它（之前讨论了去除异常的ibeacon精度提高）
%That is to say, when iBeacon devices cannot receive signal strengths (Broken)
\begin{table}[htbp]
\caption{ACCURACY OF POSITIONING SYSTEM IN EXPERIMENT 2 WITH AN IBEACON LOST}
%  what is 900ms explaining???????????????????  %100ms 和 600ms  已修正
\begin{center}
\begin{tabular}{|l|c|c|}
\hline
...
corresponding results of the same record in different datasets. (??? meaning? %每个dataset（raw或者transformed)赋予
%不同的权重 共同决定同一条记录是否predict错误  比如w1=0.6 w2=0.3 w3=0.1  低于0.9则正确 如果prediction是（1，0,0）则 ....这样的
...
detection. In the meantime, we considered one misclassification as a true detection. (so what shall we do next? %in the meantime,..........解释)
